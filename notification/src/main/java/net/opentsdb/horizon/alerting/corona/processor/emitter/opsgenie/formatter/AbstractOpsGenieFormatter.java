...
abstract class AbstractOpsGenieFormatter<
        A extends Alert,
        V extends AlertView,
        ...
        implements Formatter<MessageKit, OpsGenieAlert>
{
    ...
    protected AbstractOpsGenieFormatter(final String user,
                                        final String source,
                                        final String templateName)
    {
        ...
        final PebbleEngine engine = new PebbleEngine.Builder()
                .loader(loader)
                .build();
        this.noteTemplate = engine.getTemplate(templateName);
    ...
    /**
     * Builds alert alias, one on the form for identification in OpsGenie.
     *
     * @param view {@link MessageKitView} instance
     * @return alert alias.
     */
    public static String buildAlias(final MessageKitView<?, ?> view)
    {
    ...
    private String buildDescription(final MessageKitView<?, ?> view)
    {
        ...
        if (keys == null || keys.length == 0) {
            groupString = "<strong>Grouped on the alert level</strong>.";
        } else {
            groupString = "<strong>Grouped by</strong>:<br/>" +
                             ...
                             final ViewType... types)
    {
        ...
        for (ViewType type : types) {
            appendAlertDescriptions(sb, type, messageKitView.getViews(type));
        }
        return sb.toString();
    }
    ...
    public OpsGenieAlert format(final MessageKit messageKit)
    {
        ...
        return OpsGenieAlert.builder()
                ...
                .setAlias(buildAlias(messageKitView))
                .setUser(user)
                .setSource(source)
